...
import type { SosIncident, SosIncidentStatus, SosSeverity } from "@/features/safety/api/sos.types";
import { useFranchiseSosIncidentsList } from "../api/franchiseSos.queries";
import {
  SOS_ACTOR_LABELS,
  SOS_SEVERITY_LABELS,
  SOS_STATUS_LABELS,
...
export function FranchiseSosIncidentsListPage() {
  ...
  const { data, isLoading, isError } = useFranchiseSosIncidentsList(table.listParams);
  ...
  const columns: Column<SosIncident>[] = [
    ...
    },
    {
      ...
      cell: (row) => (
        <Link
          href={`/franchise/sos/incidents/${row.id}`}
  ...
  return (
    <div className="animate-fade-up">
      <PageHeader
        ...
        breadcrumb={["Franchise", "Opération", "SOS", "Incidents"]}
        actions={
          <Link
            href="/franchise/sos"
            className="text-sm font-medium text-teal hover:underline"
      ...
      <DataTable
        ...
        rowKey={(row) => row.id}
        isLoading={isLoading}
        exportFileName="incidents-sos-franchise"
