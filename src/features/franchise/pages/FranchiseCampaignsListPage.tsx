...
import { useFranchiseCampaigns } from "../api/marketing.queries";

const CHANNEL_LABELS: Record<FranchiseCampaign["channel"], string> = {
  push: "Push",
  sms: "SMS",
  in_app: "In-app",
  email: "Email",
...
const STATUS_LABELS: Record<FranchiseCampaign["status"], string> = {
  running: "En cours",
  scheduled: "Planifiée",
  completed: "Terminée",
  draft: "Brouillon",
...
export function FranchiseCampaignsListPage() {
  const [statusFilter, setStatusFilter] = useState<FranchiseCampaign["status"] | "all">(
    "all"
  );
  ...
  const { data, isLoading, isError } = useFranchiseCampaigns(table.listParams);

  const rows = data?.data ?? [];
  const meta = data?.meta;

  const columns: Column<FranchiseCampaign>[] = [
    {
      id: "name",
      header: "Campagne",
      cell: (c) => (
  ...
  return (
    <div className="animate-fade-up">
      ...
      <DataTable
        ...
        rowKey={(c) => c.id}
        isLoading={isLoading}
        exportFileName="campagnes-franchise"
