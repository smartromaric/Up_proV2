...
import { useFranchiseBanners } from "../api/marketing.queries";
...
export function FranchiseBannersListPage() {
  const [statusFilter, setStatusFilter] = useState<FranchiseBanner["status"] | "all">(
    "all"
  );
  ...
  const { data, isLoading, isError } = useFranchiseBanners(table.listParams);

  const rows = data?.data ?? [];
  const meta = data?.meta;

  const columns: Column<FranchiseBanner>[] = [
    {
      id: "title",
      header: "Bannière",
      cell: (b) => (
  ...
  return (
    <div className="animate-fade-up">
      ...
      <TableFiltersBar
        ...
        searchPlaceholder="Titre, emplacement…"
      ...
      <DataTable
        ...
        rowKey={(b) => b.id}
        isLoading={isLoading}
        exportFileName="bannieres-franchise"
