...
import { enrichTripWithScope, filterTripsByScope } from "./tripsScope";
...
function buildCatalog(): Trip[] {
  ...
  const rows: Trip[] = seed.map((t, i) => enrichTripWithScope(t, i));
  for (let i = 0; i < 95; i++) {
...
export function filterTrips(rows: Trip[], query: ListQuery): Trip[] {
  ...
  list = filterTripsByScope(list, query.franchise_id, query.partner_id);
  return list;
}
